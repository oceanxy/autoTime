...
(function($, window) {
  $.autoTime = {
    ...
    layout: function() {
      ...
      //设置刻度尺播放区域宽度
      this.availableWidth = this.lineWidth - (this.caller.find('.time-left').width() + this.caller.find('.time-right')
        .width() + this.caller.find('.time-oper').width() + 5)
      ...
      for(var j = 0; j < this.range[0].length; j++) {
        ...
        if(j < this.range[0].length - 1) {
          this.caller.find('.time-graduationCon')
            .append('<li class=\'time-graduation\' data-time=\'' + this.range[0][j] + '\'><i></i><p>' + disFont + '</p></li><li class=\'time-layout\'></li>')
        } else {
          this.caller.find('.time-graduationCon')
            .append('<li class=\'time-graduation\' data-time=\'' + this.range[0][j] + '\'><i></i><p>' + disFont + '</p></li>')
        }
      }
      //刻度尺定位及刻度区域划分
    ...
    animationFactory: function(index, isRestart) {
      ...
      _this.caller.find('.time-bg span').animate({left: index * (_this.interval + 30)}, delay, function() {
        ...
        _this.caller.find('.time-graduation')
          .eq(_this.prevIndex)
          .find('p')
          .animate({'fontSize': 12}, 300)
          .css('color', '#333333')
        _this.caller.find('.time-graduation')
          .eq(_this.currentIndex)
          .find('p')
          .animate({'fontSize': 14}, 300)
          .css('color', '#ff8400')
